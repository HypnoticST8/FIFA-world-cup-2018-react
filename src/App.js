...
import * as JSONdata from './data/data_full.json';
...
class App extends Component {
    ...
    render() {
        ...
        var renderGroups = groupKeys.map((e, index) => {
            return (
                <Route path={`/group${index + 1}`} key={`${groups[e].name}`}
                    render={ () =>
                        <GroupMatchesTable key={"GroupMatchesTable " + index} groups={groups[e]} teams={teams} {...this.props} />
                    }
                />
            )
        });
